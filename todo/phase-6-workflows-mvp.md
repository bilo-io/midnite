...
- [ ] P7 — Live run streaming over the first `WebSocketGateway` (emit `WorkflowEvent`s; swap polling for a WS subscription)
- [ ] P8 — Logic nodes (`logic.if` with true/false ports, `logic.setData`) + `{{expr}}` templating
- [ ] P9 — Credential vault (AES-256-GCM, `encryptionKeyEnv`) + OAuth2 start/callback; HTTP node moves to credential references
- [ ] P10 — Integration executors: Slack, Google Docs/Sheets, Email (SMTP → Gmail OAuth)
- [ ] P11 — Polish: drag-from-palette, minimap/zoom, autosave, run-history replay, templates, CLI `workflow` commands
