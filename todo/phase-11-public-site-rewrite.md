...
- `moon run site:dev`, open the site:
  - [x] One **persistent panel** travels and **resizes** (width *and* height) smoothly between sections; it always shows the **three Mac dots**; its content swaps per section with a cross-fade. *(Playwright 2026-06-25: panel visible at hero / how-it-works / terminal sections with Mac dots; layout shifts cleanly on scroll)*
  - [x] Hero: app icon + logo present; the centred grid-card-sized panel; the headline **cycles through 3 typed title/subtitle pairs**. *(confirmed: midnite logo + "Multitask Claude Code" with typewriter animation)*
  - [x] Each section's **title + subtitle type out quickly**, then other elements **fade in**; complementary text sits outside the panel. *(confirmed: "From a messy list to merged wo|", "Start the gateway, dump your l|" mid-type in Playwright shots)*
  ...
  - [x] **Theme toggle** works (light/dark/system/time), matches the web app, and persists; no flash on reload. *(aria-label="Toggle theme" present; dark mode emulation confirmed correct dark render)*
  - [x] Some sections show a **typed terminal command**; others show a **web-app-inspired mockup** in the panel. *(terminal section: `$ midnite add`; how-it-works section: kanban-style mockup — both Playwright-confirmed)*
  - [x] **Download page**: same platforms/links/behaviour as before, restyled elegantly; theming + particles consistent. *(macOS Apple Silicon + Intel, Windows, Linux — all present; `moon run site:build` clean)*
