...
- [x] **Hook auth survives the restart**: the per-session hook secret was in-memory, so a reattached `claude` session's Stop/PreToolUse callbacks would fail auth (a *completed* run would hang `wip`→abandoned). The secret **hash** is now persisted (`hook_secrets` table + [`HookSecretRepository`](../packages/gateway/src/terminal/hook-secret.repository.ts)) and rehydrated by `ApprovalService.verifySecret` on a cache miss, so reattached hooks authenticate; the row is cleared on session end / discard.
