...
- `moon run gateway:dev` (with a couple of tasks/sessions running), then `midnite watch` (via `moon run cli:dev -- watch`):
  ...
  - [x] ✅ Keyboard nav moves focus between panels/columns; moving a focused task's status updates both the TUI and the gateway (confirm in the web board). (E1/E2 + D1 + B2 now **programmatically covered** by `Dashboard.integration.test.tsx` (PR #209) — drives the real `<Dashboard>` via simulated stdin against a mocked gateway, asserting the move PATCH, nav retargeting, log-session switch, and live WS reduction; no longer TTY-gated.)
