...
- [x] shared: `tasks.nextRetryAt` (nullable) on the task contract + `agent.retryBackoffBaseMs` (10s default) / `agent.maxBackoffMs` (5min) config. `base=0` = instant retry (pre-Phase-53 behaviour preserved).
- [x] gateway: exponential backoff + **full jitter** (`random(0, min(base·2^retryIndex, cap))`) in a pure `computeBackoffMs` helper; a retryable failure re-queues with a persisted `nextRetryAt`, and `listReadyTodoTasks(now)` SQL-skips a `todo` task until its window elapses. Migration `0065` adds `tasks.next_retry_at`.
...
## Retire scheduling · add real-endpoint workflow examples · demo API

- [x] **Removed the cron/`schedule` workflow trigger and the web Schedules facade** (redundant with workflows + routines). `shared`: dropped `'schedule'` from `TRIGGER_TYPES`/`RUN_TRIGGER_SOURCES`, removed `ScheduleTriggerSchema`, the `trigger.schedule` node type, the summary `cron`/`timezone` fields, and the `defaultTimezone`/`schedulerTickMs` config knobs. Gateway: deleted `WorkflowScheduler` + `croner` dep, pruned schedule branches in service/repo/run-report. Web: deleted the `/schedules` route, `schedule-*` components, `lib/schedules`/`schedule-runs`/`cron`, and pruned the schedule option from the create modal, node-config panel, toolbar, table, and trigger badge. CLI: dropped the `schedule (cron)` trigger label. Seed templates `daily-digest`/`daily-standup` converted to `manual`; `scheduled-task-cleanup`/`github-pr-ready-check` removed; the `scheduling` template category dropped.
- [x] **New `/playground` gateway demo API** (`playground/` module, auth-exempt via `isAuthExemptPath`): `echo` on GET/POST/PUT/PATCH/DELETE (reflects method/query/headers/body) + canned `/playground/items` and `/playground/latest`. Gives workflow HTTP nodes a live, zero-setup target that showcases each REST method's input/output.
- [x] **Self-origin SSRF allowance** (`workflows/lib/self-origin.ts`): `http.request` now permits the gateway's own origin (`workflows.webhookBaseUrl`) even on loopback, so example workflows hit `/playground/*` and `/tasks` out of the box; other loopback/private targets stay blocked (still gated by `allowLoopbackHttp`).
- [x] **Real-endpoint example workflows**: fixed the 3 dead-URL web starter templates (now hit `/playground/*`), added an **HTTP methods showcase** starter, and added gateway seeds **branch-on-status** (logic.branch true/false routing) + **task-digest** (gateway self-integration over `/tasks`). Fixed `webhook-relay` to forward to `/playground/echo`. Cleared the `api.example.com` placeholders.
- [x] Verified: `moon run :typecheck` + `:lint` green; shared (698) / gateway (2016) / cli / web (1089) tests pass; end-to-end demo drove the playground echo + HTTP-method-showcase run against a live gateway.

## Reinstate the workflow `schedule` (cron) trigger

- [x] **Re-added `schedule` as a first-class workflow trigger** alongside the newer `task-event` trigger (kept). `shared`: restored `'schedule'` in `TRIGGER_TYPES`/`RUN_TRIGGER_SOURCES`, `ScheduleTriggerSchema` (`cron` + `timezone` default `UTC`), the `trigger.schedule` node type, the summary `cron`/`timezone` fields, and the `workflows.defaultTimezone`/`schedulerTickMs` config knobs. Fixtures + `trigger`/`workflow`/`node-types` schema tests cover it again.
- [x] **Gateway**: re-added `WorkflowScheduler` (single gateway-owned tick loop, `croner`) wired into `WorkflowsModule`; `WorkflowsRepository.listScheduledEnabledRows()` is the enabled-cron ready-set; `WorkflowsService` summary carries `cron`/`timezone`; run-report labels schedules as `schedule (<cron>)`. **Removed the now-counterproductive legacy `schedule→manual` read coercion** (real schedule triggers now hydrate natively) and replaced its regression test with a schedule-hydration test.
- [x] **CLI**: restored the `schedule (cron)` trigger label in `workflow list`.
- [x] **Web**: restored the Schedule option in the create modal, the node-config `ScheduleFields` (recurrence presets + raw cron + timezone + next-runs preview, backed by `lib/cron.ts`), the toolbar cron summary, the trigger badge, and the workflows-table "Schedule" section (sorted most-frequent-first). Re-added the `scheduling` template category. **The old `/schedules` facade route stays retired** — this reinstates only the trigger, not the standalone Schedules page.
- [x] Seed templates `daily-digest` (weekdays 08:00) + `daily-standup` (weekdays 09:00) restored to `schedule` triggers under the `scheduling` category.
- [x] Verified in a worktree: `moon run :typecheck` + `:lint` green (0 errors); shared / gateway (1999) / cli / web (1086) tests pass, including the restored ScheduleFields + scheduler tests.

## 2026-07-19 — Login polish: glow fixes, last-used method, entry cascade, neuro-cloud (feature/login-polish-neuro)

- [x] **Form**: `Collapse` grew an opt-in `bleed` prop (releases the inner `overflow-hidden` once the expand settles) so the email/password fields' gradient-glow halo is no longer clipped; inputs now paint a solid `bg-background` (no transparency); SSO buttons light the gradient glow on hover in per-provider palettes (Google = the default rainbow conic, GitHub = a new grayscale `gradient-border--mono` override); "Continue with email" wears a static border in the active brand accent gradient (`.accent-gradient-border`, `--accent-gradient` with solid-primary fallback).
- [x] **Last-used login method**: stored in localStorage (`midnite.lastLoginMethod`, validated on read — `lib/last-login-method.ts`); recorded on SSO click + successful email sign-in; the matching button arrives lit (`gradient-border--always` / `--lit` halo) with a small "Last used" tag.
- [x] **Entry cascade** (`use-auth-intro.ts`): once per session (sessionStorage), desktop-only, motion-gated — starfield fades in → logo fades in at the hero centre → a caret blinks beside it → the wordmark types out → the pair glides home (FLIP transform, measured once) → the title starts typing → the form cascades in (`.auth-form-cascade`, heading from above, rest staggered from below). Skips (seen / mobile / reduced motion) land on 'done' immediately.
- [x] **Neuro-cloud starfield** (`constellation-background.tsx` rewrite): ~5× the particles (cap 230→1150); constellations became branching, winding *thought paths* fired segment-by-segment by a travelling pulse with flaring, pulsating neuron glows; the cursor is a gravity well (attraction + orbital swirl, hard-core repulsion so nothing reaches the centre, home-spring restore); clicking detonates a shockwave ring that shoves neurons outward and excites 3–5 long-lived, slowly-decaying thought paths radiating from the click. Reduced-motion static frame preserved (stars + pre-lit paths, no listeners/RAF).
- [x] Tests: `use-auth-intro` timeline/skip specs, `last-login-method` round-trip/validation, SSO last-used highlight + click-record specs; existing auth suites updated and green (web unit 1147 pass; the 1 command-palette failure is pre-existing on main). `ui`+`web` typecheck + lint green. Verified visually via Playwright (intro beats, hover glows, un-clipped focus glow, gravity well, shockwave paths, light theme).

## 2026-07-19 — Login tweaks: brand-gradient fallback + gather/release neuro-cloud (feature/login-tweaks)

- [x] **Brand-gradient fallback**: `--brand-gradient` (a static CSS copy of the brand preset — blue 217/80 → violet 263/70 → rose 347/75 with the theme-aware lightness ramp, `:root` + `.dark`) now backs `.text-accent-gradient`, `.auth-hero-title`, and `.accent-gradient-border(--lit)` when the user's accent is a *solid* (no `--accent-gradient` set) — the login headings/title/email border always render the gradient instead of degrading to flat primary.
...
## 2026-07-22 — Phase 81 A–E: desktop title bar & window chrome (feature/p81-desktop-title-bar)

- [x] **Frameless macOS window** (`desktop/src/main/index.ts`): `titleBarStyle: 'hidden'` + traffic lights inset at `{ x: 16, y: 14 }` (centred in the 44px bar) + `backgroundColor: '#09090b'`, gated `process.platform === 'darwin'` — Windows/Linux keep the stock frame. The gateway-failure data-URL page carries its own drag strip.
- [x] **Window-controls IPC**: `WindowChromeBridge` contract in `shared/src/window-chrome.ts` (platform · frameless · `onFullscreenChange` · `onFocusChange` · `setBackgroundColor`); main forwards fullscreen/focus transitions and validates the `#rrggbb` backing retint (`main/window-chrome.ts` + `window-chrome/channels.ts`); preload exposes `midniteDesktop.windowChrome`, `frameless` single-sourced from the window options via `--window-frameless`.
- [x] **`<TitleBar>` in `@midnite/shell`** (`title-bar.tsx`): whole-strip drag region with `no-drag` slot wrappers, traffic-light clearance collapsing in fullscreen (bridge events — `env(titlebar-area-*)` is unpopulated on mac), blur dimming, publishes `--titlebar-h` (mirrors `--update-banner-h`); `AppFrame` gained a `titleBar` slot + rail offset. `<TitleBarDragStrip>` for full-viewport overlays. RTL specs pin framed/frameless/fullscreen/blurred; boundary tests green.
...
## 2026-07-23 — Fix: drag-to-Done leaked the agent slot (pool wedged → "no free agent slot")

- [x] **Root cause**: a board drag of a running task to Done/Abandoned (and a task delete) goes through plain `updateStatus`/`deleteTask`, which never tells the runner — the session stayed alive, the run timeout stayed armed, and the pool slot stayed busy forever. The watchdog that reclaims orphaned slots only sweeps on the scheduler tick, which never starts when `agent.poolEnabled` is off (the default, and the manual-drag workflow's config) — so after `agent.pool` such drags every slot was leaked and the next drag-to-WIP 409'd with "no free agent slot" until a gateway restart.
- [x] **Fix**: the scheduler's existing `TaskEventBus` subscription (active regardless of `poolEnabled`) now reaps a slot-holding task on `task.updated`→`done`/`abandoned` and on `task.deleted`, deferred one macrotask so the hook-driven `markDone → complete()` path settles first (the reap then no-ops). Reap = `reclaimOrphanedSlot`: clear timeout, kill session, free slot. Documented in `docs/LIFECYCLE.md` (§2, "Terminal-move reap").
- [x] Gates: gateway 2216 (4 new integration tests: drag→done, drag→abandoned, delete, hook-path no-op) · `gateway:typecheck` + `gateway:lint` green.

## 2026-07-23 — Fix: task stuck in "waiting" after answering a tool-permission modal

- [x] **Root cause**: when Claude asks a tool-permission question (the `allow`/`allow for this session`/`deny` modal), the PreToolUse hook blocks in `ApprovalService.requestDecision` and the Notification hook parks the task in `waiting`. Answering the modal resolves the held hook so Claude continues **mid-turn with no fresh prompt** — but nothing drove `waiting → wip`. The only resume triggers were the UserPromptSubmit hook (needs a typed prompt) and the PreToolUse hook (which already fired, as a no-op, *before* the wait was set). So the task stayed in "waiting" even though Claude was actively processing, only recovering if/when Claude happened to make another tool call.
- [x] **Fix**: `ApprovalService.settle` (the single choke point for user-resolve via both gateways + the timeout fail-safe) now calls `TasksService.resumeFromWaiting(sessionId)` for every resolution that hands Claude a decision (`allow`/`allow-session`/`deny`/`timeout`) — never for `expired`/`ask` (request aborted, session/connection gone). `resumeFromWaiting` is idempotent (only a live `needs-input` wait flips), so it's safe on all paths. `TasksService` injected `@Optional()` into `ApprovalService` (TasksModule is already imported by TerminalModule). Documented in `docs/LIFECYCLE.md` (§2 signal table + "Approval-resolve resume" note).
- [x] Gates: gateway 2221 (5 new approval-resume tests: allow / allow-session+deny / timeout resume; abort + clearSession do NOT) · `gateway:typecheck` + `gateway:lint` green.
