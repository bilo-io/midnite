...
- [x] Verified: full `:typecheck`/`:lint`/`:test`/`:build` green (9 shared + 9 gateway brainstorm tests, incl. synthesize / fail-on-no-ideas / mode-switch re-synthesis reusing ideas); live REST smoke against a throwaway gateway (create defaults, contributor lens, patch, 400 empty prompt / bad mode, 404 unknown)
