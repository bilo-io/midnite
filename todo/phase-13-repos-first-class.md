...
- ◐ **F · Surface repo in UI** — ✅ **repo chip on task cards** (`RepoChip`) + the thread already shows it (PR #66); the optional **per-repo status** dashboard widget (in-flight agents / queue depth per repo) remains → Phase 7 Theme C. **S.**
