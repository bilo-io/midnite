...
### D2. Navigation niceties — **S** — ✅ DONE (2026-06-26)
...
- [x] **Minimap** — ✅ (2026-06-26) a small always-on overview pinned bottom-right of the canvas: the whole 34×22 map with per-room accent-tinted outlines, agent dots (status-coloured; **red** on attention), the player dot, and a rectangle tracking the follow-camera's current viewport. Pure geometry (world↔minimap) in [`lib/office/minimap.ts`](../packages/web/lib/office/minimap.ts) (unit-tested); the scene draws it as a container tracking `cameras.main.worldView` each frame (scaled `1/ZOOM`) — robust under the scrolling follow-camera — with the panel fill re-tinting on light/dark flip. (Unblocked once the larger scrolling map + camera-follow landed, PR #206/#174.)
