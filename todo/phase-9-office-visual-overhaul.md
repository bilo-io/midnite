...
- [ ] ⏳ Drive rooms + fixtures from the Tiled object layer — still gated on Phase 8 A1 (external assets); this is the constant-driven version. **Per-room wall tints ✅ (PR #230, 2026-06-26)** — walls are now nudged toward each room's accent (`roomForWall` + `wallTint`), theme-aware; only the Tiled-driven layer remains deferred. See [done.md](done.md).
...
### B1. Distinct agent characters — **M** — ✅ DONE (2026-06-26, PR #208)
...
- [x] **Upgrade to real art:** swap the procedural variants for **MetroCity** (CC0) and/or **pixel-agents'** 6 bundled characters, or LimeZu's character generator — at the same registry seam. *(2026-06-26: pixel-agents 112×96 sheets (CC0/MIT) now power all characters — `preload()` in both scenes, `ensureCharTextures()` extracts per-direction/pose frames; ROBOT_VARIANTS→5, CHAR_H→32, 3-frame walk cycle)*
...
- [x] ✅ (2026-06-26, PR #208) **Activity-driven poses** — procedural `RobotPose` textures (`typing` / `raised` / `celebrate`) per variant registered in `ensureOfficeTextures`; full `applyPose()` state machine: `completed→celebrate`, `waiting/blocked→raised`, `running→typing`, idle→neutral. Also: `provider: LlmProvider` on `SessionSummary` + `OfficeAgent`; `providerAccent()` in `textures.ts`; provider-typed badge colour; player colour tint picker in `CharacterPicker` + persisted `playerTint` store field. Sub-agents linked near parent deferred (needs gateway sub-agent data).
