...
- [ ] **Remaining (uses the helper above):** backfill the rest with `installMockFetch` — `market-asset`/`market-watchlist` (multi-endpoint), `agents`/`throughput`/`usage`/`system-monitor`/`workflows`/`councils`/`all-projects` widgets, `boardroom-panel`. Infra now exists, so each is a small add.
