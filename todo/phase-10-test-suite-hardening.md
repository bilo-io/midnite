...
- [ ] **Remaining (uses the helper above):** backfill the other self-fetching widgets with `installMockFetch` — `market-asset`/`market-watchlist`, `sessions`/`agents`/`throughput`/`usage`/`system-monitor` widgets, `boardroom-panel`. Infra now exists, so each is a small add.
