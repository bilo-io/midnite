...
- [ ] **Remaining (uses the helper above):** the **chart/multi-endpoint** widgets still un-storied — `throughput`/`usage`/`system-monitor` (bar charts off `Date.now()` — need a pinned clock to assert), `market-asset`/`market-watchlist` (multi-endpoint), and `boardroom-panel`. Each needs a bit more than the list-widget pattern.
