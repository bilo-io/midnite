...
8. **Cross-repo template commit** → the `bug_report.md` file must be added in the **`midnite-app`** repo (this phase can't commit there); the content is authored + documented here so it's a paste-and-commit in the other repo. *(Flagged — not blocking the button, which works with or without the template file.)* **✅ Delivered 2026-07-19:** authored template opened as [`bilo-io/midnite-app#1`](https://github.com/bilo-io/midnite-app/pull/1) (ready PR) — `.github/ISSUE_TEMPLATE/bug_report.md`, name + `bug`/`from-app` labels matching `site-links.ts`.
