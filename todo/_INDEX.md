...
|-------|--------|------|----------|---|--------|--------|
| [82 · Full i18n coverage sweep (en-GB + fr-FR)](phase-82-i18n-full-coverage.md) | ◻ TODO | 0/40 | `░░░░░░░░░░` | 0% | — | A B C D E F |
...
- ◻ **A** — Gate flip & catalog machinery (ESLint allowlist → generated `I18N_EXEMPT` exception list, default-on; catalogs split to `messages/<locale>/<namespace>.json`; progress meter) — runs first
...
- ◻ **F** — Close-out (absorb Phase 79's verification, full fr-FR walkthrough + screenshots, exception list at zero)
...
- ✅ **A** — Frameless macOS window (main process: `titleBarStyle: 'hidden'` + inset traffic lights + `backgroundColor`, darwin-gated; failure-page drag strip)
...
- ◐ **E** — Lock screen & auth drag coverage ✅ + packaged-app verification pass (outstanding)
...
- ✅ **D** — Priority-surface translation: nav (feature/category labels + footer chrome), settings (sidebar + appearance headers), board (columns + empty), auth (login + SSO, ICU `{provider}`), common (confirm-dialog); en-GB + full hand fr-FR, de/es fall back until E; deep leaf strings → E tail (PR #512)
- ✅ **E** — Catalog tooling + gates: `web:i18n-validate` key-parity task (orphan/drift + complete-locale parity) + `i18n-extract` worklist + `eslint-plugin-i18next` no-hardcoded-string gate scoped to migrated files + meta sidecars + README (PR #513). ⏳ MT-seed of de-DE/es-ES deferred (empty → en-GB fallback; no offline MT)
