...
// Hold the cascade until the prompt composer has landed, matching DashboardTiles.
...
export function RecentProjects({
  ...
  // Tasks grouped by project, then by status — computed once per data change.
  ...
  return (
    <section className="space-y-3">
      <div
        className="cascade-item flex items-baseline justify-between gap-2"
        style={{ animationDelay: `${COMPOSER_SETTLE_MS + 4 * 70}ms` }}
      >
        ...
        {recent.map((project, index) => {
          ...
          return (
            <div
              ...
              style={{ animationDelay: `${COMPOSER_SETTLE_MS + (index + 5) * 70}ms` }}
            >
              <button
                type="button"
              ...
              >
                {/* Accent line + corner glow tinted to the project color. */}
                <span
                  aria-hidden
                  className="absolute inset-x-0 top-0 h-px"
                  style={{
                    backgroundImage:
                      'linear-gradient(to right, transparent, var(--proj-color), transparent)',
                  }}
                />
                <span
                  aria-hidden
                    ...
                    {legend.map((c) => (
                      ...
                      >
                        <span
                          ...
                          style={{ background: `hsl(var(${c.hueVar}))` }}
                        />
                        {c.label}
