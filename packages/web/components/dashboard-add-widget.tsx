...
/**
 * Header "+" that opens a catalogue of widgets not yet on the board. Shares the
    ...
    <div ref={ref} className="relative">
      ...
      {open && (
        <div className="absolute right-0 z-50 mt-2 w-72 overflow-hidden rounded-lg border bg-popover shadow-lg">
          ...
          {catalogue.length === 0 ? (
            <p className="px-3 py-6 text-center text-sm text-muted-foreground">All widgets added.</p>
          ) : (
            <ul className="max-h-80 overflow-auto p-1">
              {catalogue.map(({ type, label, description, icon: Icon }) => (
                <li key={type}>
                  <button
                    type="button"
                    onClick={() => add(type)}
