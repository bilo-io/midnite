...
/**
 * Phase 64 Theme D — remote teammates in the 3D office. Renders the same presence
 * store slice the 2D office does (Theme C), scoped to the active 3D scene: each
 * peer is a Phase-63-style low-poly figure (tinted) with a drei `<Html>` name
 * plate + an ephemeral emote bubble, eased toward its reported position via the
 * shared `interpStep`. Peer positions arrive as 2D world pixels and convert to 3D
 * units (`presencePxToUnit`). Meshes are `frustumCulled` (three's default), so a
 * peer in an unseen room costs no draw calls while the minimap still shows them.
 */
