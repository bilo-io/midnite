...
import type { PrCheckState, PrState, PrStatus } from '@midnite/shared';
...
export function PrStatusChip({
  ...
  const { icon, label, colour } = resolve(status);
  return (
    <span
...
function resolve(status: PrStatus): { icon: React.ReactNode; label: string; colour: string } {
  const stateLabel = STATE_LABEL[status.state];
  const checksLabel = status.checks !== 'none' ? ` · CI ${status.checks}` : '';
  const label = `PR #${status.number} — ${stateLabel}${checksLabel}`;

  if (status.state === 'merged') {
    return { icon: <GitMerge className="h-3 w-3" />, label, colour: 'bg-purple-500/15 text-purple-600 dark:text-purple-400' };
  }
  if (status.state === 'closed') {
    return { icon: <XCircle className="h-3 w-3" />, label, colour: 'bg-muted text-muted-foreground' };
  }
  if (status.state === 'draft') {
    return { icon: <Circle className="h-3 w-3" />, label, colour: 'bg-muted text-muted-foreground' };
  }
  // open — colour by checks
  const checkColour = CHECK_COLOUR[status.checks];
  const CheckIcon = CHECK_ICON[status.checks];
  return { icon: <CheckIcon className="h-3 w-3" />, label, colour: checkColour };
}
