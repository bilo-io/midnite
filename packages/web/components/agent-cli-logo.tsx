...
// Single-path brand marks (24×24), sourced from simple-icons (CC0). Claude →
// Anthropic's Claude mark, Gemini → Google Gemini, Codex → OpenAI.
const CLAUDE_PATH =
...
/** A small brand mark for each agent CLI. Claude/Gemini carry brand colour;
 *  OpenAI (Codex) tracks `currentColor` so it reads in both themes. */
export function AgentCliLogo({ cli, className }: { cli: AgentCli; className?: string }) {
  const gradId = useId();
  const cls = cn('h-4 w-4', className);

  if (cli === 'claude') {
    return (
      <svg viewBox="0 0 24 24" className={cls} fill="#D97757" role="img" aria-hidden>
        <path d={CLAUDE_PATH} />
      </svg>
    );
  }
  ...
  if (cli === 'gemini') {
    return (
      <svg viewBox="0 0 24 24" className={cls} role="img" aria-hidden>
        <defs>
        ...
        <path d={GEMINI_PATH} fill={`url(#${gradId})`} />
      </svg>
    );
  }
