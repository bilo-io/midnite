...
/**
 * Phase 62 G — the Digests feed. Two-pane master-detail: the left rail lists
 * recent digests (date, headline, counts); the right pane renders the selected
 * digest's structured detail with task deep-links + markdown export. Selection
 * lives in the `?id=` query param so a digest is deep-linkable (and global search
 * routes straight to one).
 */
export function DigestsView() {
  ...
  useEffect(() => {
    if (!selectedId && list.length > 0) {
      router.replace(`/digests?id=${encodeURIComponent(list[0]!.id)}`, { scroll: false });
    }
  }, [selectedId, list, router]);
