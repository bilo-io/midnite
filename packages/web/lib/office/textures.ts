...
/**
 * No-op guard kept for call-site compatibility. The tileset PNG is now loaded
 * in the scene's preload() via `this.load.image(TILESET_KEY, '/office/floor-tiles.png')`,
 * so the texture is already registered before create() runs.
 */
export function ensureOfficeTileset(scene: Phaser.Scene): void {
  if (scene.textures.exists(TILESET_KEY)) return;
  // Fallback: nothing to create — the PNG must have been loaded in preload().
}
