...
export class AgentRunnerService implements OnModuleInit {
  ...
  async completeWithChecks(taskId: string, prUrl: string): Promise<void> {
    ...
    const taskAfterChecks = this.tasks.getTask(taskId);
    if (autoFix.enabled && taskAfterChecks.fixAttempts < autoFix.maxAttempts) {
      this.tasks.incrementFixAttempts(taskId);
      this.tasks.recordCheckEvent(taskId, 'checks.fix.started');
      this.logger.log(
        `auto-fix attempt ${taskAfterChecks.fixAttempts + 1}/${autoFix.maxAttempts} for task ${taskId}`,
      );
    ...
    if (autoFix.enabled && taskAfterChecks.fixAttempts >= autoFix.maxAttempts) {
      ...
      this.logger.warn(
        `auto-fix budget exhausted (${taskAfterChecks.fixAttempts}/${autoFix.maxAttempts}) for task ${taskId}`,
      );
    }
    this.tasks.markWaiting(taskId);
    this.complete(taskId);
