 ...
 * services, exposed via `POST /chat/command` + `/chat/preview`. Theme C: the
 * read-only query answerer ({@link ChatQueryService}) + its `POST /chat/query`
 * endpoint. Theme D: the intent service's routing policy (local-preferred,
 * budget-capped) reads the Phase 50 hard budget cap via UsageModule. Theme F: the
 * confirm-gate + the undo log ({@link ChatCommandsRepository} → `chat_commands`) +
 * {@link ChatUndoService} (`POST /chat/undo`); commands are audited via the
 * `@Global` AuditModule. Imports AgentModule (LlmService, BreakdownService),
 * TasksModule, ProjectsModule, UsageModule; DB + AuditService come from their
 * `@Global` modules.
 */
@Module({
  imports: [AgentModule, TasksModule, ProjectsModule, UsageModule],
  controllers: [ChatController, ChatQueryController],
  providers: [
    ChatIntentService,
    ChatCommandService,
  ...
  exports: [ChatIntentService, ChatCommandService, ChatUndoService, ChatQueryService],
})
