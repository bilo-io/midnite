...
export interface GatewayClient {
  ...
  /** Phase 69 E — reopen a terminal task (`done`/`abandoned` → `todo`), clearing
   *  bindings + retry state and re-blocking dependents. Rejects (400) for a
   *  non-terminal task; the message surfaces in the thrown error. */
...
export function createClient(baseUrl: string, token?: string): GatewayClient {
  ...
  return {
    ...
    async reopenTask(id: string): Promise<Task> {
      return TaskSchema.parse(
        await request(`/tasks/${encodeURIComponent(id)}/reopen`, { method: 'POST' }),
      );
    },
